  ...
  return (
    <div className="app-content">
      ...
      <section className="table-card">
        ...
        {filteredProducts.length === 0 ? <div className="empty-state"><Heart size={28} /><h3>{favoritesOnly ? "Aucun favori pour le moment" : "Aucune référence trouvée"}</h3><p>{favoritesOnly ? "Ajoutez une référence depuis le catalogue pour la retrouver ici." : "Essayez un autre nom, laboratoire ou filtre de disponibilité."}</p></div> : <table className="product-table"><thead><tr><th>Produit</th><th>Référence</th><th>Disponibilité</th><th>Quantité</th><th /></tr></thead><tbody>{filteredProducts.map((product) => { const quantity = quantityFor(product.id); return <motion.tr key={product.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }}><td><div className="product-name-cell"><span className="product-thumb">{product.initials}</span><span><strong>{product.name}</strong><span>{product.lab}</span></span></div></td><td><span className="table-ref">{product.reference}</span></td><td><span className={`stock-label ${product.stock === "Disponible" ? "stock-available" : product.stock === "Stock limité" ? "stock-limited" : "stock-unavailable"}`}>{product.stock}</span></td><td>{quantity > 0 ? <QuantitySelector value={quantity} onDecrease={() => updateQuantity(product.id, quantity - 1)} onIncrease={() => updateQuantity(product.id, quantity + 1)} /> : <Button small variant="quiet" onClick={() => addToCart(product)} disabled={product.stock === "Indisponible"}><ShoppingBasket size={13} /> Ajouter</Button>}</td><td><div className="row-actions"><button className={`favorites-toggle ${favorites.has(product.id) ? "is-favorite" : ""}`} aria-label={favorites.has(product.id) ? `Retirer ${product.name} des favoris` : `Ajouter ${product.name} aux favoris`} onClick={() => toggleFavorite(product.id)}><Heart size={15} fill={favorites.has(product.id) ? "currentColor" : "none"} /></button></div></td></motion.tr>})}</tbody></table>}
      </section>
